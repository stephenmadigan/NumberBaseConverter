...
public class NumberBaseGUI implements ChangeListener {
	...
	public static String colorToBinary(Color c) { //converts alpha+RBG color into 32 bit binary.
		String binaryNumber = "";
		String temp = "" + Integer.toBinaryString(c.getAlpha());
		while (temp.length() < 8) {
			temp = "0" + temp;
		}
		System.out.println(temp);
		binaryNumber += temp;
		temp = "" + Integer.toBinaryString(c.getRed());
		while (temp.length() < 8) {
			temp = "0" + temp;
		}
		System.out.println(temp);
		binaryNumber += temp;
		temp = "" + Integer.toBinaryString(c.getGreen());
		while (temp.length() < 8) {
			temp = "0" + temp;
		}
		System.out.println(temp);
		binaryNumber += temp;
		temp = "" + Integer.toBinaryString(c.getBlue());
		while (temp.length() < 8) {
			temp = "0" + temp;
		}
		System.out.println(temp);
